<script>
	...
	const checkStreep = () => {
		drinkerBekend ? addStreep(drankjes, drinkerId) : alert('onbekende drinker');
		drinker = '';
		drinkerBekend = false;
		drankjes = 1;
	};
...
<div class="flex m-1">
	<input
		...
		class:bg-rose-100={!drinkerBekend && drinker !== ''}
		class:bg-green-100={drinkerBekend}
		class:bg-white={drinker == ''}
		on:input={() => getDrinker()}
	/>
...
{#if showNieuweDrinker}
	<Modal on:close={() => addNieuwDrinker()} title="Nieuwe Drinker">
		<div slot="body"><NieuweDrinker {drinker} on:close={() => addNieuwDrinker()}/></div>
	</Modal>
{/if}
